...
class mysqlMark extends dbbaseMark{
	...
	public function connect($host, $user, $password, $database, $charset = 'utf8', $pconnect = 0) {
		...
		if (!$link_id) MarkPHP::markError('mysql connect error!');
		mysql_query('SET NAMES ' . $charset, $link_id);
		if (!mysql_select_db($database, $link_id)) MarkPHP::markError('database is not exist!');
		return $link_id;
	}
